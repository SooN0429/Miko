...
def test(model, target_test_loader, test_flag):
    ...
    with open('D:\\Tang\\target_model_training_record_from_' + opt.source_model +'.txt', 'a') as f:
    	f.write('\ntest accuracy = ')
...
if __name__ == '__main__':
    ...
    source_save_parameter_path = 'D:\\Tang\\create_datasets\\source_model_para\\' + opt.extracted_layer + '\\' + opt.source_model + '\\' +str(opt.lr) + '\\'
    source_save_parameter_add = os.path.join(source_save_parameter_path + str(opt.save_parameter_path_name))

    save_parameter_path = 'D:\\Tang\\create_datasets\\target_model_para\\' + opt.extracted_layer + '\\' + opt.source_model + '_to\\' + opt.source_class + '\\' +str(opt.lr) + '\\'
    ...
    if not os.path.exists(source_save_parameter_path):
        os.makedirs(source_save_parameter_path)
    ...
    model = model.cuda()
    model.load_state_dict(torch.load(source_save_parameter_add, map_location='cuda:0'))
    print(source_save_parameter_add)
    ...
    save_parameter_add = os.path.join(save_parameter_path + 'color' + str(opt.save_parameter_path_name))
