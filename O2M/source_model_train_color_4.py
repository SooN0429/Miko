...
def test(model, target_test_loader, test_flag):
    ...
    with open('D:\\Tang\\color\\source_model_training_record'+str(opt.epoch)+opt.save_parameter_path_name+'.txt', 'a') as f:
    	f.write('\ntest accuracy = ')
...
if __name__ == '__main__':
    ...
    path_source_train = os.path.join(opt.train_root_path, opt.source_dir, 'source_train_ColorJitter_feature.npy')
    path_source_train_label = os.path.join(opt.train_root_path, opt.source_dir, 'source_train_ColorJitter_feature_label.npy')
    ...
    model = models4_2.Transfer_Net(CFG['n_class'])
    model = model.cuda()
