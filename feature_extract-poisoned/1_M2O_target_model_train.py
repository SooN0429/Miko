# 使用來源端訓練好的模型，並用 "目標端資料(之前抽取的特徵)" 進行微調，最後進行測試並記錄(可視作目標端模型的測試)。
# 兩個來源端訓練好的模型權重，用權重加權的方式(所有模型權重都會進行融合，包括特徵提取層的權重，
# 雖然特徵提取層的部份權重都一樣，但重點在特徵萃取層以外的權重是有經過source資料訓練的，融合是為了有經過訓練的權重)
...
parser.add_argument('--source_model_1', type=str, default="None")
parser.add_argument("--para_1", type=float, default=0.0001, help="initial learning rate")
parser.add_argument('--source_model_2', type=str, default="None")
parser.add_argument("--para_2", type=float, default=0.0001, help="initial learning rate")
...
def test(model, target_test_loader, test_flag):
    ...
    with open('D:\\Tang\\target_model_training_record_from_' + opt.extracted_layer + '.txt', 'a') as f:
    	f.write('\ntest accuracy = ')
...
if __name__ == '__main__':
    ...
    source_save_parameter_path_1 = 'D:\\Tang\\create_datasets\\source_model_para\\' + opt.extracted_layer + '\\' + opt.source_model_1 + '\\' +str(opt.lr) + '\\'
    source_save_parameter_add_1 = os.path.join(source_save_parameter_path_1 + str(opt.save_parameter_path_name))
    source_save_parameter_path_2 = 'D:\\Tang\\create_datasets\\source_model_para\\' + opt.extracted_layer + '\\' + opt.source_model_2 + '\\' +str(opt.lr) + '\\'
    source_save_parameter_add_2 = os.path.join(source_save_parameter_path_2 + str(opt.save_parameter_path_name))

    save_parameter_path = 'D:\\Tang\\create_datasets\\target_model_para\\' + opt.extracted_layer + '\\' + opt.source_model_1 + '_' + opt.source_model_2 + '_to\\' + opt.source_class + '\\' +str(opt.lr) + '\\'
    ...
    if not os.path.exists(source_save_parameter_path_1):
        os.makedirs(source_save_parameter_path_1)
    if not os.path.exists(source_save_parameter_path_2):
        os.makedirs(source_save_parameter_path_2)
    ...
    model_1.load_state_dict(torch.load(source_save_parameter_add_1))
    print(source_save_parameter_add_1)
    model_2 = models.Transfer_Net(CFG['n_class'])
    model_2 = model_2.cuda()
    model_2.load_state_dict(torch.load(source_save_parameter_add_2))
    print(source_save_parameter_add_2)
    model = models.Transfer_Net(CFG['n_class'])
    model = model.cuda()
    for name, parameters in model.named_parameters():
        model.state_dict()[name] = model_1.state_dict()[name] * opt.para_1 + model_2.state_dict()[name] * opt.para_2
    ...
    #scheduler=torch.optim.lr_scheduler.StepLR(optimizer,step_size=20,gamma=0.16)
    with open('D:\\Tang\\target_model_training_record_from_' + opt.extracted_layer + '.txt', 'a') as f:
        f.write(opt.source_model_1)
        f.write(opt.source_model_2)
        f.write('\nfruit = ')
        f.write(source_name)
        f.write('\nlayer = ')
