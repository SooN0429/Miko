...
# 基礎日誌目錄 ((實驗數據日誌
base_log_dir="/mnt/F/Vincent_v1.0/Model_Fusion_LOGs/Model_Fusion_alpha=0.5_beta=0.5_Epoch_25_log"
...
for current_model_index in "${!models[@]}"; do
    ...
    # 創建當前模型的實驗名稱和日誌目錄 ((這邊是前面提到的實驗數據日誌下的子集
    experiment_name="alpha=0.5_beta=0.5_Epoch_25_${model_name}<-models_gray<-ori_1"
    # experiment_name="ZipIt!_alpha=0.2_${model_name}<-models_gray<-ori_4"
    log_dir="${base_log_dir}/${model_name}"
    results_excel="${log_dir}/results_2025_${experiment_name}.xlsx"
    ...
    for fruit1 in "${fruits[@]}"; do
        ...
        for fruit2 in "${fruits[@]}"; do
            ...
            #sed -i "s/model_0 = models[0-9_]*.Transfer_Net/model_0 = models.Transfer_Net/g" /mnt/F/Vincent_v1.0/CL_MAL/model_fusion_2025.py
            #sed -i "s/model_1 = models[0-9_]*.Transfer_Net/model_1 = ${model_name}.Transfer_Net/g" /mnt/F/Vincent_v1.0/CL_MAL/model_fusion_2025.py
            #sed -i "s/model = models[0-9_]*.Transfer_Net/model = models.Transfer_Net/g" /mnt/F/Vincent_v1.0/CL_MAL/model_fusion_2025.py

            # 修改Python文件中的模型引用 (modelsx_x<-models) ((以下兩行目標是"自動"替換目標模型(上面提到的11個models)，具體自動替換可以從哪裡看到有在替換：model_fusion_2025.py的1306行替代、1322行，主要替換目標模型而已，source不動，若需要改成自動替換source models，上面三個註解就要打開，可以從：model_fusion_2025.py的1314行看到替換過程
            sed -i "s/model_0 = models[0-9_]*.Transfer_Net/model_0 = ${model_name}.Transfer_Net/g" /mnt/F/Vincent_v1.0/CL_MAL/model_fusion_2025.py
            sed -i "s/model = models[0-9_]*.Transfer_Net/model = ${model_name}.Transfer_Net/g" /mnt/F/Vincent_v1.0/CL_MAL/model_fusion_2025.py
            
            
            # 執行實驗命令範本 ((此處為gray示範
            #((主要調整這邊的source_train_feature、source_train_feature_label：dataset的輸入features
            #save_parameter_path_name_1：目標模型的參數(目前是用學姊的"第一次"訓練(如 XX_0_modelXX.pkl)的參數)
            #save_parameter_path_name_2：來源模型的參數(基本不用動)
            #fusion_alpha：調整fusion共享區塊，這邊的數字越小，代表來源模型的權重越大(與論文相反，論文是數值越大，來源模型的權重越大)
            #fusion_beta：個人區塊的權重，0.5不用調整
            #fusion_layers：除了backbone都會融合，個人區塊與共享區塊都會融合
            #statistical_method：消融實驗
            pipenv run python /mnt/F/Vincent_v1.0/CL_MAL/model_fusion_2025.py \
                --source_model_1 "$fruit1" \
                --source_train_feature source_train_gray_feature.npy \
                --source_train_feature_label source_train_gray_feature_label.npy \
                ...
                --test_root_path /mnt/F/Vincent_v1.0/create_datasets/feature_extract/dataset/gray_test_image/ \
                --source_dir "${fruit1}_to_/to_${fruit1}/source/train/" \
                --target_test_dir "${fruit1}_to_/to_${fruit1}/sn_200_sp_200/target/test/" \
                --batch_size 8 \
                --lr 0.0001 \
                --epoch 25 \
                --save_parameter_path_name "alpha=0.5_beta=0.5_${model_name}<-models_gray<-ori_1" \
                --save_parameter_path_name_1 "epoch50_gray_0_model${model_num}.pkl" \
